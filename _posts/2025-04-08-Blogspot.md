...
Finding this on Google it says there's a promising C code for doing alteration of sudo with `sudoedit`.
...
A handy tool that we can use for our password attacks is [NetExec](https://github.com/Pennyw0rth/NetExec), which can also be used for other protocols such as SMB, LDAP, MSSQL, and others. We recommend reading the official documentation for this tool to become familiar with it.
...
```shell-session
c0derpwner@htb[/htb]$ nmap -sT -p22,3306 10.129.202.64
...
```shell-session
c0derpwner@htb[/htb]$ ssh -L 1234:localhost:3306 ubuntu@10.129.202.64
...
```shell-session
c0derpwner@htb[/htb]$ netstat -antp | grep 1234
...
```shell-session
c0derpwner@htb[/htb]$ nmap -v -sV -p1234 localhost
...
```shell-session
c0derpwner@htb[/htb]$ ssh -L 1234:localhost:3306 -L 8080:localhost:80 ubuntu@10.129.202.64
```
...
```shell-session
c0derpwner@htb[/htb]$ ssh -D 9050 ubuntu@10.129.202.64
```
...
```shell-session
c0derpwner@htb[/htb]$ tail -4 /etc/proxychains.conf
...
```shell-session
c0derpwner@htb[/htb]$ proxychains nmap -v -sn 172.16.5.1-200
...
```shell-session
c0derpwner@htb[/htb]$ proxychains nmap -v -Pn -sT 172.16.5.19
...
```shell-session
c0derpwner@htb[/htb]$ proxychains msfconsole
...
```shell-session
c0derpwner@htb[/htb]$ proxychains xfreerdp /v:172.16.5.19 /u:victor /p:pass@123
